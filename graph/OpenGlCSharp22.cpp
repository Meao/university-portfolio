...
#
define MAX_EMBOSS(GLfloat) 0.008 f
...
# define MAX_EXTENSION_SPACE 10240# define MAX_EXTENSION_LENGTH 256
...
int LoadGLTextures() {
  ...
  if (Image = auxDIBImageLoad("Data/Bump.bmp")) {
    glPixelTransferf(GL_RED_SCALE, 0.5 f);
    glPixelTransferf(GL_GREEN_SCALE, 0.5 f);
    glPixelTransferf(GL_BLUE_SCALE, 0.5 f);
    ...
    glPixelTransferf(GL_RED_SCALE, 1.0 f);
    glPixelTransferf(GL_GREEN_SCALE, 1.0 f);
    glPixelTransferf(GL_BLUE_SCALE, 1.0 f);
  } else status = false;
  if (Image) {
    if (Image - > data) delete Image - > data;
    delete Image;
...
GLvoid ReSizeGLScene(GLsizei width, GLsizei height) {
  ...
  gluPerspective(45.0 f, (GLfloat) width / (GLfloat) height, 0.1 f, 100.0 f);
...
void doLogo(void) {
  ...
  glVertex3f(0.23 f, -0.4 f, -1.0 f);
  glTexCoord2f(1.0 f, 0.0 f);
  glVertex3f(0.53 f, -0.4 f, -1.0 f);
  glTexCoord2f(1.0 f, 1.0 f);
  glVertex3f(0.53 f, -0.25 f, -1.0 f);
  glTexCoord2f(0.0 f, 1.0 f);
  glVertex3f(0.23 f, -0.25 f, -1.0 f);
  glEnd();
  if (useMultitexture) {
    glBindTexture(GL_TEXTURE_2D, multiLogo);
    glBegin(GL_QUADS);
    ...
    glVertex3f(-0.53 f, -0.4 f, -1.0 f);
    glTexCoord2f(1.0 f, 0.0 f);
    glVertex3f(-0.33 f, -0.4 f, -1.0 f);
    glTexCoord2f(1.0 f, 1.0 f);
    glVertex3f(-0.33 f, -0.3 f, -1.0 f);
    glTexCoord2f(0.0 f, 1.0 f);
    glVertex3f(-0.53 f, -0.3 f, -1.0 f);
    glEnd();
  }
  glDepthFunc(GL_LEQUAL);
}
...
bool doMesh1TexelUnits(void) {
  ...
  glLoadIdentity();
  glRotatef(-yrot, 0.0 f, 1.0 f, 0.0 f);
  glRotatef(-xrot, 1.0 f, 0.0 f, 0.0 f);
  glTranslatef(0.0 f, 0.0 f, -z);
  ...
  xrot += xspeed;
  yrot += yspeed;
  if (xrot > 360.0 f) xrot -= 360.0 f;
  if (xrot < 0.0 f) xrot += 360.0 f;
  if (yrot > 360.0 f) yrot -= 360.0 f;
  if (yrot < 0.0 f) yrot += 360.0 f;
