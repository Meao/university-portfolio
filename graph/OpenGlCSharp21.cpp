...
void TimerInit(void) {
  ...
  if (!QueryPerformanceFrequency((LARGE_INTEGER * ) & timer.frequency)) {
    ...
    timer.resolution = 1.0 f / 1000.0 f;
    ...
    timer.resolution = (float)(((double) 1.0 f) / ((double) timer.frequency));
...
float TimerGetTime() {
  ...
  if (timer.performance_timer) {
    ...
    return ((float)(time - timer.performance_timer_start) * timer.resolution) * 1000.0 f;
  } else {

    return ((float)(timeGetTime() - timer.mm_timer_start) * timer.resolution) * 1000.0 f;
  }
}
...
GLvoid BuildFont(GLvoid) {
  ...
  for (loop1 = 0; loop1 < 256; loop1++) {
    float cx = float(loop1 % 16) / 16.0 f;
    float cy = float(loop1 / 16) / 16.0 f;

    glNewList(base + loop1, GL_COMPILE);
    glBegin(GL_QUADS);
    glTexCoord2f(cx, 1.0 f - cy - 0.0625 f);
    glVertex2d(0, 16);
    glTexCoord2f(cx + 0.0625 f, 1.0 f - cy - 0.0625 f);
    glVertex2i(16, 16);
...
GLvoid glPrint(GLint x, GLint y, int set,
  const char * fmt, ...) {
  char text[256];
  va_list ap;
...
GLvoid ReSizeGLScene(GLsizei width, GLsizei height) {
  ...
  glOrtho(0.0 f, width, height, 0.0 f, -1.0 f, 1.0 f);
...
int InitGL(GLvoid) {
  ...
  glClearColor(0.0 f, 0.0 f, 0.0 f, 0.5 f);
  glClearDepth(1.0 f);
...
int DrawGLScene(GLvoid) {
  ...
  for (loop1 = 0; loop1 < lives - 1; loop1++) {
    glLoadIdentity();
    glTranslatef(490 + (loop1 * 40.0 f), 40.0 f, 0.0 f);
    glRotatef(-player.spin, 0.0 f, 0.0 f, 1.0 f);
    ...
    glVertex2d(-5, 5);
    glEnd();
    glRotatef(-player.spin * 0.5 f, 0.0 f, 0.0 f, 1.0 f);
    glColor3f(0.0 f, 0.75 f, 0.0 f);
    glBegin(GL_LINES);
    glVertex2d(-7, 0);
    glVertex2d(7, 0);
    glVertex2d(0, -7);
    ...
    for (loop2 = 0; loop2 < 11; loop2++) {
      ...
      if ((loop1 < 10) && (loop2 < 10)) {
        ...
        if (hline[loop1][loop2] && hline[loop1][loop2 + 1] && vline[loop1][loop2] && vline[loop1 + 1][loop2]) {
          glBegin(GL_QUADS);
          glTexCoord2f(float(loop1 / 10.0 f) + 0.1 f, 1.0 f - (float(loop2 / 10.0 f)));
          glVertex2d(20 + (loop1 * 60) + 59, (70 + loop2 * 40 + 1));
          glTexCoord2f(float(loop1 / 10.0 f), 1.0 f - (float(loop2 / 10.0 f)));
          glVertex2d(20 + (loop1 * 60) + 1, (70 + loop2 * 40 + 1));
          glTexCoord2f(float(loop1 / 10.0 f), 1.0 f - (float(loop2 / 10.0 f) + 0.1 f));
          glVertex2d(20 + (loop1 * 60) + 1, (70 + loop2 * 40) + 39);
          glTexCoord2f(float(loop1 / 10.0 f) + 0.1 f, 1.0 f - (float(loop2 / 10.0 f) + 0.1 f));
  ...
  if (hourglass.fx == 1) {
    glLoadIdentity();
    glTranslatef(20.0 f + (hourglass.x * 60), 70.0 f + (hourglass.y * 40), 0.0 f);
    glRotatef(hourglass.spin, 0.0 f, 0.0 f, 1.0 f);
    glColor3ub(rand() % 255, rand() % 255, rand() % 255);
    glBegin(GL_LINES);
    glVertex2d(-5, -5);
    glVertex2d(5, 5);
  ...
  glLoadIdentity();
  glTranslatef(player.fx + 20.0 f, player.fy + 70.0 f, 0.0 f);
  glRotatef(player.spin, 0.0 f, 0.0 f, 1.0 f);
  glColor3f(0.0 f, 1.0 f, 0.0 f);
  glBegin(GL_LINES);
  glVertex2d(-5, -5);
  ...
  for (loop1 = 0; loop1 < (stage * level); loop1++) {
    glLoadIdentity();
    glTranslatef(enemy[loop1].fx + 20.0 f, enemy[loop1].fy + 70.0 f, 0.0 f);
    glColor3f(1.0 f, 0.5 f, 0.5 f);
    glBegin(GL_LINES);
    glVertex2d(0, -7);
    glVertex2d(-7, 0);
    glVertex2d(-7, 0);
    ...
    glVertex2d(0, -7);
    glEnd();
    glRotatef(enemy[loop1].spin, 0.0 f, 0.0 f, 1.0 f);
...
int WINAPI WinMain(HINSTANCE hInstance,
  ...
  while (!done) {
    if (PeekMessage( & msg, NULL, 0, 0, PM_REMOVE)) {
      ...
      while (TimerGetTime() < start + float(steps[adjust] * 2.0 f)) {}
      ...
      player.spin += 0.5 f * steps[adjust];
      if (player.spin > 360.0 f) {
        player.spin -= 360;
      }

      hourglass.spin -= 0.25 f * steps[adjust];
      if (hourglass.spin < 0.0 f) {
        hourglass.spin += 360.0 f;
      }
