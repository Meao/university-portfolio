...
void ProcessHelix() {
  ...
  glTranslatef(0, 0, -50);
  glRotatef(angle / 2.0 f, 1, 0, 0);
  glRotatef(angle / 3.0 f, 0, 1, 0);
  ...
  for (phi = 0; phi <= 360; phi += 20.0) {
    for (theta = 0; theta <= 360 * twists; theta += 20.0) {
      v = (phi / 180.0 f * 3.142 f);
      u = (theta / 180.0 f * 3.142 f);

      x = float(cos(u) * (2.0 f + cos(v))) * r;
      y = float(sin(u) * (2.0 f + cos(v))) * r;
      z = float(((u - (2.0 f * 3.142 f)) + sin(v)) * r);
...
void RenderToTexture() {
  ...
  glClearColor(0.0 f, 0.0 f, 0.5 f, 0.5);
...
void DrawBlur(int times, float inc) {
  ...
  for (int num = 0; num < times; num++) {
    glColor4f(1.0 f, 1.0 f, 1.0 f, alpha);
...
void Update(DWORD milliseconds) {
  ...
  angle += (float)(milliseconds) / 5.0 f;
}
