...
MotionUnderGravitation * motionUnderGravitation =
  new MotionUnderGravitation(Vector3D(0.0 f, -9.81 f, 0.0 f));
...
MassConnectedWithSpring * massConnectedWithSpring =
  new MassConnectedWithSpring(2.0 f);

float slowMotionRatio = 10.0 f;
float timeElapsed = 0;
...
GLvoid glPrint(float x, float y, float z,
  const char * fmt, ...) {
  float length = 0;
  char text[256];
  va_list ap;
...
void Update(DWORD milliseconds) {
  ...
  if (g_keys - > keyDown[VK_F2] == TRUE)
    slowMotionRatio = 1.0 f;

  if (g_keys - > keyDown[VK_F3] == TRUE)
    slowMotionRatio = 10.0 f;

  float dt = milliseconds / 1000.0 f;

  dt /= slowMotionRatio;

  timeElapsed += dt;

  float maxPossible_dt = 0.1 f;
...
void Draw(void) {
  ...
  for (float x = -20; x <= 20; x += 1.0 f) {
    glVertex3f(x, 20, 0);
    glVertex3f(x, -20, 0);
  }

  for (float y = -20; y <= 20; y += 1.0 f) {
    glVertex3f(20, y, 0);
    glVertex3f(-20, y, 0);
  }
  ...
  glPrint(-5.0 f, 14, 0, "Time elapsed (seconds): %.2f", timeElapsed);
  glPrint(-5.0 f, 13, 0, "Slow motion ratio: %.2f", slowMotionRatio);
  glPrint(-5.0 f, 12, 0, "Press F2 for normal motion");
  glPrint(-5.0 f, 11, 0, "Press F3 for slow motion");
