...
GLvoid BuildFont(GLvoid) {
  ...
  for (int loop = 0; loop < 95; loop++) {
    float cx = float(loop % 16) / 16.0 f;
    float cy = float(loop / 16) / 8.0 f;

    glNewList(base + loop, GL_COMPILE);
    glBegin(GL_QUADS);
    glTexCoord2f(cx, 1.0 f - cy - 0.120 f);
    glVertex2i(0, 0);
    glTexCoord2f(cx + 0.0625 f, 1.0 f - cy - 0.120 f);
    glVertex2i(16, 0);
    glTexCoord2f(cx + 0.0625 f, 1.0 f - cy);
    glVertex2i(16, 16);
    glTexCoord2f(cx, 1.0 f - cy);
    glVertex2i(0, 16);
    glEnd();
    glTranslated(10, 0, 0);
    glEndList();
...
GLvoid glPrint(GLint x, GLint y,
  const char * string, ...) {
  char text[256];
  va_list ap;
...
GLvoid InitObject(int num) {
  ...
  object[num].distance = -(float(rand() % 4001) / 100.0 f);
  object[num].y = -1.5 f + (float(rand() % 451) / 100.0 f);

  object[num].x = ((object[num].distance - 15.0 f) / 2.0 f) - (5 * level) - float(rand() % (5 * level));
  ...
  if (object[num].texid == 1) {
    object[num].dir = 3;
    object[num].x = float(rand() % int(object[num].distance - 10.0 f)) + ((object[num].distance - 10.0 f) / 2.0 f);
    object[num].y = 4.5 f;
  }
  ...
  if (object[num].texid == 2) {
    object[num].dir = 2;
    object[num].x = float(rand() % int(object[num].distance - 10.0 f)) + ((object[num].distance - 10.0 f) / 2.0 f);
    object[num].y = -3.0 f - float(rand() % (5 * level));
  }
...
void Selection(void) {
  ...
  gluPickMatrix((GLdouble) mouse_x, (GLdouble)(viewport[3] - mouse_y), 1.0 f, 1.0 f, viewport);

  gluPerspective(45.0 f, (GLfloat)(viewport[2] - viewport[0]) / (GLfloat)(viewport[3] - viewport[1]), 0.1 f, 100.0 f);
...
void Update(DWORD milliseconds) {
  ...
  roll -= milliseconds * 0.00005 f;

  for (int loop = 0; loop < level; loop++) {
    if (object[loop].rot == 1)
      object[loop].spin -= 0.2 f * (float(loop + milliseconds));

    if (object[loop].rot == 2)
      object[loop].spin += 0.2 f * (float(loop + milliseconds));

    if (object[loop].dir == 1)
      object[loop].x += 0.012 f * float(milliseconds);

    if (object[loop].dir == 0)
      object[loop].x -= 0.012 f * float(milliseconds);

    if (object[loop].dir == 2)
      object[loop].y += 0.012 f * float(milliseconds);

    if (object[loop].dir == 3)
      object[loop].y -= 0.0025 f * float(milliseconds);

    if ((object[loop].x < (object[loop].distance - 15.0 f) / 2.0 f) && (object[loop].dir == 0) && !object[loop].hit) {
      miss += 1;
      object[loop].hit = TRUE;
    }

    if ((object[loop].x > -(object[loop].distance - 15.0 f) / 2.0 f) && (object[loop].dir == 1) && !object[loop].hit) {
      miss += 1;
      object[loop].hit = TRUE;
    }

    if ((object[loop].y < -2.0 f) && (object[loop].dir == 3) && !object[loop].hit) {
      miss += 1;
      object[loop].hit = TRUE;
    }

    if ((object[loop].y > 4.5 f) && (object[loop].dir == 2))
      object[loop].dir = 3;
  }
}
...
void Explosion(int num) {
  float ex = (float)((object[num].frame / 4) % 4) / 4.0 f;
  float ey = (float)((object[num].frame / 4) / 4) / 4.0 f;

  glBindTexture(GL_TEXTURE_2D, textures[5].texID);
  glBegin(GL_QUADS);
  glTexCoord2f(ex, 1.0 f - (ey));
  glVertex3f(-1.0 f, -1.0 f, 0.0 f);
  glTexCoord2f(ex + 0.25 f, 1.0 f - (ey));
  glVertex3f(1.0 f, -1.0 f, 0.0 f);
  glTexCoord2f(ex + 0.25 f, 1.0 f - (ey + 0.25 f));
  glVertex3f(1.0 f, 1.0 f, 0.0 f);
  glTexCoord2f(ex, 1.0 f - (ey + 0.25 f));
  glVertex3f(-1.0 f, 1.0 f, 0.0 f);
  glEnd();
...
void DrawTargets(void) {
  ...
  for (int loop = 0; loop < level; loop++) {
    ...
    if (object[loop].hit) {
      ...
      glRotatef(object[loop].spin, 0.0 f, 0.0 f, 1.0 f);
...
void Draw(void) {
  ...
  glBegin(GL_QUADS);
  glTexCoord2f(1.0 f, roll / 1.5 f + 1.0 f);
  glVertex3f(28.0 f, +7.0 f, -50.0 f);
  glTexCoord2f(0.0 f, roll / 1.5 f + 1.0 f);
  glVertex3f(-28.0 f, +7.0 f, -50.0 f);
  glTexCoord2f(0.0 f, roll / 1.5 f + 0.0 f);
  glVertex3f(-28.0 f, -3.0 f, -50.0 f);
  glTexCoord2f(1.0 f, roll / 1.5 f + 0.0 f);
  glVertex3f(28.0 f, -3.0 f, -50.0 f);

  glTexCoord2f(1.5 f, roll + 1.0 f);
  glVertex3f(28.0 f, +7.0 f, -50.0 f);
  glTexCoord2f(0.5 f, roll + 1.0 f);
  glVertex3f(-28.0 f, +7.0 f, -50.0 f);
  glTexCoord2f(0.5 f, roll + 0.0 f);
  glVertex3f(-28.0 f, -3.0 f, -50.0 f);
  glTexCoord2f(1.5 f, roll + 0.0 f);
  glVertex3f(28.0 f, -3.0 f, -50.0 f);

  glTexCoord2f(1.0 f, roll / 1.5 f + 1.0 f);
  glVertex3f(28.0 f, +7.0 f, 0.0 f);
  glTexCoord2f(0.0 f, roll / 1.5 f + 1.0 f);
  glVertex3f(-28.0 f, +7.0 f, 0.0 f);
  glTexCoord2f(0.0 f, roll / 1.5 f + 0.0 f);
  glVertex3f(-28.0 f, +7.0 f, -50.0 f);
  ...
  glBegin(GL_QUADS);
  glTexCoord2f(7.0 f, 4.0 f - roll);
  glVertex3f(27.0 f, -3.0 f, -50.0 f);
  glTexCoord2f(0.0 f, 4.0 f - roll);
  glVertex3f(-27.0 f, -3.0 f, -50.0 f);
  glTexCoord2f(0.0 f, 0.0 f - roll);
  glVertex3f(-27.0 f, -3.0 f, 0.0 f);
  glTexCoord2f(7.0 f, 0.0 f - roll);
  glVertex3f(27.0 f, -3.0 f, 0.0 f);
  glEnd();
  ...
  glMatrixMode(GL_MODELVIEW);
  glTranslated(mouse_x, window.bottom - mouse_y, 0.0 f);
  Object(16, 16, 8);
