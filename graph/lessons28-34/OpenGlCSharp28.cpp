...
GLuint genBezier(BEZIER_PATCH patch, int divs) {
  ...
  for (u = 1; u <= divs; u++) {
    ...
    pyold = ((float) u - 1.0 f) / ((float) divs);
...
int InitGL(GLvoid) {
  ...
  glClearColor(0.05 f, 0.05 f, 0.05 f, 0.5 f);
  glClearDepth(1.0 f);
...
int DrawGLScene(GLvoid) {
  ...
  glLoadIdentity();
  glTranslatef(0.0 f, 0.0 f, -4.0 f);
  glRotatef(-75.0 f, 1.0 f, 0.0 f, 0.0 f);
  glRotatef(rotz, 0.0 f, 0.0 f, 1.0 f);
...
int WINAPI WinMain(HINSTANCE hInstance,
  ...
  while (!done) {
    if (PeekMessage( & msg, NULL, 0, 0, PM_REMOVE)) {
      ...
      if (keys[VK_LEFT]) rotz -= 0.8 f;
      if (keys[VK_RIGHT]) rotz += 0.8 f;
      if (keys[VK_UP]) {
        divs++;
        mybezier.dlBPatch = genBezier(mybezier, divs);
        keys[VK_UP] = FALSE;
